...
/**
 * MilestoneTimeline · 里程碑时间线视图(v2 扩展版)。
 *
 * v1: 平铺列表 + 进度条。
 * v2(2026-04-21): 激活里程碑展开 Subtasks + InfoPanel,目标级周投入图(gap-audit §P1.1)。
 */

import { Check, ChevronDown, ChevronRight, Circle, Pencil, Plus, X } from "lucide-vue-next";
import { computed, ref, watch } from "vue";

import GoalWeeklyInvestChart from "@/components/goal/GoalWeeklyInvestChart.vue";
import MilestoneInfoPanel from "@/components/goal/MilestoneInfoPanel.vue";
import MilestoneSubtasks from "@/components/goal/MilestoneSubtasks.vue";
...
/** 用户手动展开/收起的 milestoneId 集合 · 未命中则默认用 activeId */
...
function isExpanded(id: string): boolean {
  if (id in manualExpanded.value) return manualExpanded.value[id];
  return id === activeId.value;
}

function toggleExpand(id: string) {
  manualExpanded.value = { ...manualExpanded.value, [id]: !isExpanded(id) };
}

// 切换 goal 时清 manualExpanded
watch(() => goals.selectedGoalId, () => { manualExpanded.value = {}; });
...
<template>
  <div v-if="selectedGoal" class="fl-timeline">
    ...
    <div class="fl-tl-list">
      <template v-for="m in goals.milestones" :key="m.id">
        <div
          class="fl-ms-item"
          :class="{
            'is-done': m.status === 'completed',
            'is-active': m.id === activeId,
            'is-expanded': isExpanded(m.id),
          }"
        >
          <button
            class="fl-ms-dot"
          ...
          <!-- 右侧按钮组 -->
          <div v-if="editingId !== m.id" class="fl-ms-actions">
            <button class="fl-ms-action-btn" title="编辑" @click="startEdit(m)">
              <Pencil :size="10" />
            </button>
            <button
              class="fl-ms-action-btn fl-ms-expand-btn"
              :title="isExpanded(m.id) ? '收起' : '展开'"
              @click="toggleExpand(m.id)"
            >
              <ChevronDown v-if="isExpanded(m.id)" :size="12" />
    ...
    <!-- 目标级周投入图 -->
    <GoalWeeklyInvestChart v-if="goals.milestones.length" />
  </div>
...
<style scoped>
.fl-timeline { display: flex; flex-direction: column; gap: var(--sp-4); }
...
.fl-tl-progress { font-size: var(--fs-12); color: var(--color-text-muted); }

.fl-tl-bar {
  height: 6px; border-radius: 3px;
  background: var(--color-bg-hover); position: relative; overflow: hidden;
}
.fl-tl-bar-fill {
...
.fl-tl-desc { margin: 0; font-size: var(--fs-12); color: var(--color-text-secondary); }
...
.fl-ms-item:hover { background: var(--color-bg-hover); }
...
.fl-ms-item.is-expanded { background: var(--color-bg-subtle); }
...
.fl-ms-body { display: flex; flex-direction: column; gap: 2px; flex: 1; }
.fl-ms-name { font-size: var(--fs-14); color: var(--color-text-primary); }
.fl-ms-desc { font-size: 11px; color: var(--color-text-muted); }
...
@media (max-width: 720px) {
  .fl-ms-expand-split { grid-template-columns: 1fr; gap: var(--sp-4); }
}

.fl-ms-add { display: flex; gap: var(--sp-2); margin-left: var(--sp-2); }
.fl-ms-input {
  ...
  font-size: var(--fs-12); outline: none;
}
.fl-ms-input:focus { border-color: var(--color-primary); }
.fl-ms-add-btn {
...
.fl-ms-add-btn:disabled { opacity: 0.4; cursor: not-allowed; }
...
.fl-ms-item:hover .fl-ms-actions,
.fl-ms-item.is-expanded .fl-ms-actions { opacity: 1; }
.fl-ms-action-btn {
...
.fl-ms-action-btn:hover { color: var(--color-primary); background: var(--color-bg-hover); }
.fl-ms-expand-btn { color: var(--color-text-secondary); }
